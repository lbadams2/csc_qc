...
def factor(P, use_saved_embedding=True):
    ...
    csp = dbc.factories.multiplication_circuit(4) # maybe change to 4
    ...
    # convert P from decimal to binary - P will be 6 bit number after this, dict has 0 or 1 for each p_var
    ...
    bqm_embedded = dwave.embedding.embed_bqm(bqm, embedding, target_adjacency, 3.55) # change to 3.55

    # draw samples from the QPU
    kwargs = {}
    if 'num_reads' in sampler.parameters:
        kwargs['num_reads'] = 2000
    ...
    for sample, num_occurrences, energy in response.data(['sample', 'num_occurrences', 'energy']):
        ...
        if (a, b, P) in results_dict:
            ...
            results_dict[(a, b, P)]["energy"] += energy
            results_dict[(a, b, P)]["avg_energy"] = results_dict[(a, b, P)]["energy"] / \
                results_dict[(a, b, P)]["numOfOccurrences"]
        else:
